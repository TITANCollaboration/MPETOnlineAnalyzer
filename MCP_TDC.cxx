				...
				switch (TDC & 0xF0000000) {
					case VT2_HSTART: 
						...
						printf("xxx --- CYCLE: %d --- Frequency: %.3f --- freqN: %d --- fStep: %.3f\n",
								CycleNumber, CurrentFrequency, (int)freqN, gFreqStep);

						lTDC = TDCb[++i];
						hstart_time64 =  ((((uint64_t)TDC & 0xFFFF)<<32) + lTDC); 

						printf("HSTART : CycleSN:%d  MCP TIME:%lld\n", cycleSN, hstart_time64);
					...
					case VT2_HIT1: 
						...
						if (startfound) {
							...
							multiH1++;
							printf("HIT1: Cycle1N:%d  MCP TIME:%lld\n", cycle1N, hit1_time64);
					...
					case VT2_HIT2: 
						...
						if (startfound) {
							...
							multiH2++;
							printf("HIT2: Cycle2N:%d  MCP TIME:%lld\n", cycle2N, hit2_time64);
					...
					case VT2_HSTOP:
						...
						if (startfound) {
							...
							printf("HSTOP: CyclePN:%d MCP TIME:%lld\n", cyclePN, hstop_time64);
